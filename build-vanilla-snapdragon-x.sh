...
LIVE_ISO_CONTAINER_RUNTIME="${LIVE_ISO_CONTAINER_RUNTIME:-docker}"  # docker is the contributor-proven default
LIVE_ISO_CONTAINER_FALLBACK_IMAGES="${LIVE_ISO_CONTAINER_FALLBACK_IMAGES:-}"  # v6: no silent generic-container fallback
...
MIN_FREE_GB_BEFORE_OCI_BUILD="${MIN_FREE_GB_BEFORE_OCI_BUILD:-30}"
MIN_FREE_GB_BEFORE_ISO_BUILD="${MIN_FREE_GB_BEFORE_ISO_BUILD:-20}"
MIN_FREE_GB_BEFORE_OCI_EXPORT="${MIN_FREE_GB_BEFORE_OCI_EXPORT:-24}"
...
resolve_custom_kernel_release() {
  # Resolve the exact bootable kernel release from the supplied linux-image
  # package contents, not from a truncated human version label.
  #
  # The artifacts directory is authoritative. The complete release is derived
  # from the supplied linux-image package payload, for example
  # 7.1.1-jg-1-qcom-x1e. That full release must be used for:
  #   /lib/modules/<release>
  #   /boot/vmlinuz-<release>
  #   /boot/initrd.img-<release>
  #   /live/vmlinuz-<release>
  #   /live/initrd.img-<release>
  local deb pkg path release
  local -a releases=()
  ...
  for deb in "${KERNEL_DEBS[@]:-}"; do
    ...
    # Prefer authoritative paths in the package payload.  Accept either the
    # packaged vmlinuz filename or the module directory.  Both normally encode
    # the exact uname -r string required by update-initramfs and GRUB.
    while IFS= read -r path; do
      path="${path#./}"
      case "$path" in
        boot/vmlinuz-*)
          release="${path#boot/vmlinuz-}"
          ;;
        lib/modules/*)
          release="${path#lib/modules/}"
          release="${release%%/*}"
          ;;
        *)
          continue
    ...
    done < <(dpkg-deb -c "$deb" 2>/dev/null | awk '{print $NF}')

    # Package-name fallback for packages whose archive listing omits a direct
    # boot image path.  This still retains the complete target suffix.
  ...
  [[ "${#releases[@]}" -gt 0 ]] || return 1

  mapfile -t releases < <(printf '%s\n' "${releases[@]}" | awk 'NF && !seen[$0]++' | sort)
  if [[ "${#releases[@]}" -ne 1 ]]; then
    fail "Supplied linux-image packages resolve to multiple kernel releases: ${releases[*]}"
    return 1
  fi

  release="${releases[0]}"

  # Optional operator safety constraint. By default this is empty, preserving
  # the reusable-builder contract that the user-supplied artifacts determine
  # the kernel release without requiring a script edit for each new kernel.
  if [[ -n "$EXPECTED_CUSTOM_KERNEL_RELEASE" && "$release" != "$EXPECTED_CUSTOM_KERNEL_RELEASE" ]]; then
    fail "Supplied linux-image package resolves to '$release', but EXPECTED_CUSTOM_KERNEL_RELEASE requires '$EXPECTED_CUSTOM_KERNEL_RELEASE'."
    return 1
  fi
...
patch_live_iso_grub_for_custom_dtb() {
  # Patch the GRUB template exactly as it exists at Stage 7.
  #
  # Vanilla's source tree may already contain concrete /live/vmlinuz-* and
  # /live/initrd.img-* paths rather than KERNEL_LIVE / INITRD_LIVE placeholders.
  # Therefore this function does not require placeholders. It rewrites the
  # existing linux and initrd commands inside each live menuentry to the exact
  # custom kernel release derived from the supplied linux-image package.
  local live="$1"
  local dtb_name="$2"
  ...
  # Count all live initrd commands, whether still represented by a placeholder
  # or already materialized to a concrete /live/initrd.img-* path.
  expected_entries="$(grep -oE 'initrd(efi)?[[:space:]]+(INITRD_LIVE|/live/initrd\.img-[^[:space:]}]+)' "$grub" | wc -l | awk '{print $1}')"
  if [[ "${expected_entries:-0}" -eq 0 ]]; then
    fail "The live ISO GRUB template contains no live initrd commands."
    return 1
  fi

  {
    printf 'GRUB template: %s\n' "$grub"
    printf 'Expected custom kernel: %s\n' "$custom_kernel_path"
    printf 'Expected custom initrd: %s\n' "$custom_initrd_path"
    printf 'DTB: %s\n' "$dtb_name"
    printf 'Live initrd commands before patch: %s\n' "$expected_entries"
...
# Remove an accidental standalone token while preserving the valid variable
# test: if [ "$grub_platform" = "efi" ]; then

data = re.sub(r"(?m)^[ \t]*grub_platform[ \t]*\n", "", data)

# Parse menuentry blocks with brace-depth tracking so this also works if the
# upstream template is compacted or reformatted.
...
linux_cmd = re.compile(
    r"(?P<prefix>\blinux(?:efi)?[ \t]+)"
    r"(?P<kernel>KERNEL_LIVE|LINUX_LIVE|/live/vmlinuz-[^\s]+)"
    r"(?P<args>.*?)"
    r"(?P<end>[ \t]+---)",
    re.S,
)
initrd_cmd = re.compile(
    r"(?P<indent>^[ \t]*)"
    r"(?P<prefix>initrd(?:efi)?[ \t]+)"
    r"(?P<initrd>INITRD_LIVE|/live/initrd\.img-[^\s;}]+)",
    re.M,
)
managed_dtb = re.compile(
    r"(?m)^[ \t]*(?:#[ \t]*CUSTOM_ARM64_DTB_MANAGED_BY_BUILDER[ \t]*\n)?"
...
def rewrite_linux(block: str, desired: str) -> str:
    m = linux_cmd.search(block)
    if not m:
        compact = " ".join(block.split())[:500]
        raise SystemExit(f"live menuentry lacks a parseable linux command: {compact}")
    kept = []
    for token in m.group("args").split():
        if token in controlled_exact:
            continue
        ...
        if token.startswith("modprobe.blacklist=nouveau"):
            continue
        kept.append(token)
    merged = kept + desired.split()
    replacement = m.group("prefix") + custom_kernel
    if merged:
        replacement += " " + " ".join(merged)
    replacement += m.group("end")
    return block[:m.start()] + replacement + block[m.end():]

def rewrite_initrd_and_dtb(block: str) -> str:
    # Remove only builder-managed DTB lines, then replace the existing live
    # initrd path/placeholder and insert the selected DTB immediately before it.
    block = managed_dtb.sub("", block)
    m = initrd_cmd.search(block)
    if not m:
        compact = " ".join(block.split())[:500]
        raise SystemExit(f"live menuentry lacks a parseable initrd command: {compact}")
    indent = m.group("indent")
    replacement = (
        f"{indent}# {marker}\n"
        f"{indent}devicetree /boot/dtbs/{dtb}\n"
        f"{indent}{m.group('prefix')}{custom_initrd}"
    )
    return block[:m.start()] + replacement + block[m.end():]

# A live entry is any menuentry containing both a supported live kernel command
# and a supported live initrd command. No placeholder is required.
live_entries = []
for s, e, title in entries:
    block = data[s:e]
    if linux_cmd.search(block) and initrd_cmd.search(block):
        live_entries.append((s, e, title))
...
for index, (s, e, title) in enumerate(live_entries):
    ...
    if index == 0:
        desired = diag_args
        new_title = "Install Vanilla OS 2 (Diagnostic Boot)"
    elif "Safe Graphics" in title or "Firmware Framebuffer" in title:
        desired = firmware_fb_args
        new_title = "Install Vanilla OS 2 (Firmware Framebuffer)"
    elif "Nouveau unloaded" in title or "Normal Graphics" in title:
        desired = normal_args
        new_title = "Install Vanilla OS 2 (Normal Graphics)"
    else:
        desired = normal_args
        new_title = title

    block = rewrite_linux(block, desired)
    block = rewrite_initrd_and_dtb(block)
    block = re.sub(
        r'(menuentry\s+["\']).*?(["\']\s*\{)',
...
if data.count(f"devicetree /boot/dtbs/{dtb}") < len(live_entries):
    raise SystemExit("not every live entry references the selected DTB")
...
PYGRUB
  ...
  awk -v expected="$custom_kernel_path" '$1 ~ /^linux(efi)?$/ && $2 == expected { found++ } END { exit !(found >= 3) }' "$grub" || return 1
  awk -v expected="$custom_initrd_path" '$1 ~ /^initrd(efi)?$/ && $2 == expected { found++ } END { exit !(found >= 3) }' "$grub" || return 1
  ! grep -Eq 'KERNEL_LIVE|LINUX_LIVE|INITRD_LIVE' "$grub" || return 1

  ok "Patched every live GRUB entry to the exact custom kernel and initramfs, with the selected DTB."
...
verify_live_iso_source_integration() {
  ...
  awk -v expected="/live/vmlinuz-$expected_kver" '$1 ~ /^linux(efi)?$/ && $2 == expected { found=1 } END { exit !found }' "$live/etc/config/bootloaders/grub-pc/grub.cfg" || {
    fail "GRUB template does not reference exact custom kernel: /live/vmlinuz-$expected_kver"
    failed=1
  }

  awk -v expected="/live/initrd.img-$expected_kver" '$1 ~ /^initrd(efi)?$/ && $2 == expected { found=1 } END { exit !found }' "$live/etc/config/bootloaders/grub-pc/grub.cfg" || {
...
verify_built_iso_custom_boot_artifacts() {
  ...
  if [[ -f "$grub_extract" && -s "$grub_extract" ]]; then
    ...
    awk -v expected="/live/vmlinuz-$expected_kver" '$1 ~ /^linux(efi)?$/ && $2 == expected { found=1 } END { exit !found }' "$grub_extract" || {
      fail "Generated ISO GRUB config does not reference exact custom kernel."
      warn "Verification log: $log"
      return 1
    }

    awk -v expected="/live/initrd.img-$expected_kver" '$1 ~ /^initrd(efi)?$/ && $2 == expected { found=1 } END { exit !found }' "$grub_extract" || {
...
# Return available bytes for the filesystem containing the supplied path.
...
# Fail before an expensive stage instead of allowing a long build to exhaust
# the filesystem. The message includes both required and observed capacity.
require_free_space_gb() {
  local path="$1" required_gb="$2" purpose="$3"
  local available_bytes required_bytes available_gb
  available_bytes="$(available_bytes_for_path "$path")"
  required_bytes=$((required_gb * 1024 * 1024 * 1024))
  available_gb=$((available_bytes / 1024 / 1024 / 1024))
  if (( available_bytes < required_bytes )); then
    die "Insufficient free space for $purpose. Path: $path; required: ${required_gb} GiB; available: ${available_gb} GiB. Free space or move WORKDIR/OUTPUT_DIR before retrying."
  fi
  ok "Capacity preflight passed for $purpose: ${available_gb} GiB available; ${required_gb} GiB required."
}

prepare_oci_v6_tree() {
  # v6 architecture: hardware enablement belongs in the Core OCI. The Desktop
  # image is then rebuilt from that local hardware-specific Core image.
  local core="$SOURCES_DIR/core-image" desktop="$SOURCES_DIR/desktop-image"
  local core_recipe="$core/recipe.yml" desktop_recipe="$desktop/recipe.yml"
  ...
  for f in "${KERNEL_DEBS[@]}"; do cp -a "$f" "$core/includes.container/opt/vendor-kernel/"; done
  for f in "${DTB_FILES[@]}"; do cp -a "$f" "$core/includes.container/boot/dtbs/"; done
  if [[ -d "$STAGED_QCOM_DIR" ]] && find "$STAGED_QCOM_DIR" -type f -print -quit | grep -q .; then
...
commands:
  - >-
    ...
    mkdir -p /etc/initramfs-tools/conf.d;
    echo 'MODULES=$INITRAMFS_MODULES_POLICY' > /etc/initramfs-tools/conf.d/conception-target-modules;
    rm -f "/boot/initrd.img-\$EXPECTED_KERNEL";
    update-initramfs -c -k "\$EXPECTED_KERNEL";
    test -s "/boot/initrd.img-\$EXPECTED_KERNEL";
    ln -sfn "boot/vmlinuz-\$EXPECTED_KERNEL" /vmlinuz;
    ln -sfn "boot/initrd.img-\$EXPECTED_KERNEL" /initrd.img;
    echo "\$EXPECTED_KERNEL" > /etc/conception-custom-kernel-release;
    echo "\$EXPECTED_DTB" > /etc/conception-custom-dtb
EOF_OCI_MODULE

  python3 - "$core_recipe" "$desktop_recipe" "$CORE_IMAGE_TAG" <<'PY_RECIPE'
from pathlib import Path
import re, sys
core=Path(sys.argv[1]); desktop=Path(sys.argv[2]); local_core=sys.argv[3]
cs=core.read_text()
cs=re.sub(r'(?m)^(\s*base:)\s*\S+\s*$', r'\1 ghcr.io/vanilla-os/pico:dev', cs, count=1)
...
ds=desktop.read_text()
ds=re.sub(r'(?m)^(\s*base:)\s*\S+\s*$', lambda m: f'{m.group(1)} {local_core}', ds, count=1)
desktop.write_text(ds)
PY_RECIPE
  ...
  grep -Fq 'base: ghcr.io/vanilla-os/pico:dev' "$core_recipe" || die "Core recipe was not rebased to pico:dev"
  grep -Fq 'modules/zz-conception-hardware.yml' "$core_recipe" || die "Core recipe does not include the hardware module"
  grep -Fq "base: $CORE_IMAGE_TAG" "$desktop_recipe" || die "Desktop recipe does not use local Core image $CORE_IMAGE_TAG"
  grep -Fq "EXPECTED_KERNEL='$kver'" "$module" || die "Hardware module does not contain the derived kernel release"

  python3 - "$module" <<'PY_VALIDATE'
import sys, yaml
obj=yaml.safe_load(open(sys.argv[1]))
cmd=obj['commands'][0]
if not isinstance(cmd, str) or not cmd.strip():
    raise SystemExit('generated hardware command is empty')
open(sys.argv[1]+'.command-check.sh','w').write(cmd+'\n')
PY_VALIDATE
  sh -n "$module.command-check.sh" || die "Generated hardware-module shell command failed syntax validation"
  rm -f "$module.command-check.sh"
  ok "Prepared official VanillaOS dev OCI chain with custom core kernel $kver and DTB $dtb_name."
}

build_oci_images_v6() {
  require_free_space_gb "$WORKDIR" "$MIN_FREE_GB_BEFORE_OCI_BUILD" "Core and Desktop OCI builds"
  local core="$SOURCES_DIR/core-image"
  local desktop="$SOURCES_DIR/desktop-image"
...
verify_target_oci_v6() {
  local kver dtb_name inspect_log firmware_expected=0 firmware_probe_rel="" failures=0
  kver="$(resolve_custom_kernel_release)"
  dtb_name="$(basename "$PRIMARY_DTB")"
  ...
  if [[ -d "$STAGED_QCOM_DIR" ]] && find "$STAGED_QCOM_DIR" -type f -print -quit | grep -q .; then
    firmware_expected=1
    firmware_probe_rel="$(find "$STAGED_QCOM_DIR" -type f -printf '%P\n' | LC_ALL=C sort | head -n1)"
  fi

  info "Verifying target OCI image contents with predicate-level reporting."
  : >"$inspect_log"
  ...
  verify_predicate() {
    ...
    if podman run --rm --entrypoint /bin/sh "$TARGET_OCI_IMAGE" -c "$command" >>"$inspect_log" 2>&1; then
      printf 'RESULT: PASS\n\n' >>"$inspect_log"
      ok "OCI verification: $label"
    else
      printf 'RESULT: FAIL\n\n' >>"$inspect_log"
      fail "OCI verification predicate failed: $label"
      failures=$((failures + 1))
    fi
  }
  ...
  verify_predicate "DTB metadata records selected file" "test -f /etc/conception-custom-dtb && grep -Fx '$dtb_name' /etc/conception-custom-dtb"
  if [[ "$firmware_expected" == "1" ]]; then
    verify_predicate "staged Qualcomm firmware probe exists: $firmware_probe_rel" "test -s '/usr/lib/firmware/$firmware_probe_rel' || test -s '/lib/firmware/$firmware_probe_rel'"
  else
    warn "No staged Qualcomm firmware files were supplied; firmware-presence predicate skipped."
  fi

  if (( failures > 0 )); then
    fail "Target OCI verification failed: $failures predicate(s) failed. Detailed results: $inspect_log"
  ...
  ok "Target OCI verification passed. Identity: $OCI_IMAGE_DIGEST"
}

export_target_oci_optional_v6() {
  OCI_ARCHIVE_PATH=""
  OCI_ARCHIVE_SHA256=""
  if [[ "$EXPORT_OCI_ARCHIVE" != "1" ]]; then
    info "OCI archive export disabled by default. Local image retained as $TARGET_OCI_IMAGE"
    return 0
  fi

  local archive available_bytes required_bytes
  archive="${OCI_ARCHIVE_PATH_REQUESTED:-$OUTPUT_DIR/${PROFILE}-${BUILD_DATE}-desktop.oci.tar}"
  mkdir -p "$(dirname "$archive")"
  available_bytes="$(available_bytes_for_path "$(dirname "$archive")")"
  required_bytes=$((MIN_FREE_GB_BEFORE_OCI_EXPORT * 1024 * 1024 * 1024))
  if (( available_bytes < required_bytes )); then
    warn "Skipping optional OCI export: fewer than ${MIN_FREE_GB_BEFORE_OCI_EXPORT} GiB are free on $(dirname "$archive"). ISO generation will continue."
    return 0
  fi

  info "Exporting verified target OCI archive: $archive"
  rm -f "$archive"
  if ! podman save --format oci-archive -o "$archive" "$TARGET_OCI_IMAGE"; then
    rm -f "$archive"
    warn "Optional OCI export failed. The verified local image remains available and ISO generation will continue."
    return 0
  fi
  if [[ ! -s "$archive" ]]; then
    rm -f "$archive"
    warn "Optional OCI export produced no archive. ISO generation will continue."
    return 0
  fi
  OCI_ARCHIVE_PATH="$archive"
  OCI_ARCHIVE_SHA256="$(sha256sum "$archive" | awk '{print $1}')"
  ok "Exported target OCI archive: $OCI_ARCHIVE_PATH"
}

push_target_oci_optional_v6() {
  [[ "$PUSH_TARGET_OCI_IMAGE" == "1" ]] || return 0
  case "$TARGET_OCI_IMAGE" in
    localhost/*|local/*) die "--push-target-image requires a registry-qualified target image, not $TARGET_OCI_IMAGE" ;;
  esac
  podman push "$TARGET_OCI_IMAGE" || die "Target OCI push failed: $TARGET_OCI_IMAGE"
  ok "Pushed target OCI image: $TARGET_OCI_IMAGE"
}
...
select_live_iso_helper_image() {
  local runtime="$1"
  [[ "$LIVE_ISO_CONTAINER_IMAGE" == "ghcr.io/vanilla-os/pico:dev" ]] || warn "Using operator-overridden Pico image: $LIVE_ISO_CONTAINER_IMAGE"
  "$runtime" pull "$LIVE_ISO_CONTAINER_IMAGE"
  "$runtime" image inspect "$LIVE_ISO_CONTAINER_IMAGE" >/dev/null
  "$runtime" run --rm "$LIVE_ISO_CONTAINER_IMAGE" /bin/bash -lc 'echo PICO_DEV_EXEC_OK' >/dev/null
  ok "Selected contributor-compatible live ISO helper image: $LIVE_ISO_CONTAINER_IMAGE"
}
...
EOF_MOTD
  ...
  ok "Prepared official VanillaOS live-iso tree for ARM64 custom-image installation."
}
...
Options:
  ...
  --export-oci                    Export a local OCI archive after verification (off by default).
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --export-oci) EXPORT_OCI_ARCHIVE=1; shift ;;
...
stage 8 12 "Verifying the target desktop OCI image and optionally exporting it."
verify_target_oci_v6
push_target_oci_optional_v6
export_target_oci_optional_v6

stage 9 12 "Preparing the official VanillaOS installer ISO source tree."
prepare_live_iso_v4_tree
...
Target OCI archive: ${OCI_ARCHIVE_PATH:-not exported}
Target OCI archive SHA-256: ${OCI_ARCHIVE_SHA256:-not exported}
...
archive=${OCI_ARCHIVE_PATH:+$(basename "$OCI_ARCHIVE_PATH")}
